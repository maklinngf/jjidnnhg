...
# 作者: 小龙女她爸
# 日期: 2025-08-02
# 修改日期: 2025-08-25 (增强TLS, 自定义Banner, 澄清中继)
# =================================================================================
...
setup_caddy_reverse_proxy() {
    ...
    echo ""
    echo -e "${YELLOW}重要提示: Caddy已经为您申请了SSL证书。为了让SMTP服务也使用加密，"
    echo -e "请务必重启SMTP服务以加载证书：${NC}"
    echo -e "${GREEN}sudo systemctl restart mail-smtp.service${NC}"
    echo "================================================================"
    exit 0
}
...
install_server() {
    ...
    # <<< NEW: 询问邮件域名，用于Banner和TLS证书路径 >>>
    read -p "请输入您的主邮件域名 (例如 mail.yourdomain.com): " MAIL_DOMAIN
    if [ -z "$MAIL_DOMAIN" ]; then
        echo -e "${RED}错误: 邮件域名不能为空。它将用于SMTP横幅和自动查找SSL证书。${NC}"
        exit 1
    fi
    ...
    ufw allow 587/tcp # <<< NEW: 开放STARTTLS推荐端口
    ...
    # <<< MODIFIED: app.py has been significantly updated for TLS and other improvements >>>
...
MAIL_DOMAIN = "_PLACEHOLDER_MAIL_DOMAIN_"
...
# --- Web UI and Helper Functions (No changes in this section) ---
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        return f(*args, **kwargs)
    return decorated_function
# --- Web Routes (No changes in this section) ---
...
# <<< NEW: SMTP Handler and Server Start Logic is updated for TLS, Banner, and Relay clarification >>>
class CustomSMTPHandler:
    async def handle_RCPT(self, server, session, envelope, address, rcpt_options):
        # This server is a local sink. We accept mail for any recipient.
        # This is NOT an open relay because we do not FORWARD mail to another server.
        # We simply accept it for local storage.
        app.logger.info(f"Accepting recipient: {address}")
        envelope.rcpt_tos.append(address)
        return '250 OK'

    async def handle_DATA(self, server, session, envelope):
        try:
            # The recipient list is already populated by handle_RCPT
            recipient_list = ','.join(envelope.rcpt_tos)
            process_email_data(recipient_list, envelope.content)
            return '250 OK'
...
def get_tls_context():
    # Automatically find and use Caddy's Let's Encrypt certificates
    cert_path = Path(f"/var/lib/caddy/.local/share/caddy/certificates/acme-v02.api.letsencrypt.org-directory/{MAIL_DOMAIN}/{MAIL_DOMAIN}.crt")
    key_path = Path(f"/var/lib/caddy/.local/share/caddy/certificates/acme-v02.api.letsencrypt.org-directory/{MAIL_DOMAIN}/{MAIL_DOMAIN}.key")
    
    if cert_path.is_file() and key_path.is_file():
        try:
            context = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
            context.load_cert_chain(cert_path, key_path)
            app.logger.info(f"成功加载 Caddy 证书 for {MAIL_DOMAIN}。SMTP TLS 已启用。")
            return context
        except Exception as e:
            app.logger.error(f"加载证书时出错: {e}. 将在没有 TLS 的情况下启动。")
            return None
    else:
        app.logger.warning(f"在 {cert_path} 未找到 Caddy 证书。")
        app.logger.warning("SMTP 服务器将在没有 TLS (加密) 的情况下启动。请先运行 Caddy 设置来生成证书。")
        return None
...
if __name__ == '__main__':
    ...
    controller = Controller(
        ...
        hostname='0.0.0.0',
        port=25,
        server_hostname=MAIL_DOMAIN, # <<< NEW: Set custom banner
        tls_context=tls_context      # <<< NEW: Enable STARTTLS if context is available
    )
    
    controller.start()
    app.logger.info(f"SMTP 服务器启动，监听端口 25 (Banner: {MAIL_DOMAIN})...")
...
EOF
    ...
    # No changes needed for service files
...
"
    ...
    sed -i "s#_PLACEHOLDER_MAIL_DOMAIN_#${MAIL_DOMAIN}#g" "${PROJECT_DIR}/app.py" # <<< NEW
    ...
    echo -e "1. 您的Web后台目前通过HTTP访问: ${YELLOW}http://${PUBLIC_IP}:${WEB_PORT}${NC}"
    echo -e "2. 您的SMTP服务目前 ${RED}可能未加密${NC}。请立即运行选项 '3' 来配置Caddy。"
    echo -e "3. Caddy配置完成后，它会自动申请SSL证书，请务必 ${GREEN}重启SMTP服务${NC} 以加载证书："
    echo -e "   ${YELLOW}sudo systemctl restart mail-smtp.service${NC}"
    echo "================================================================"
}

# --- 主逻辑 ---
clear
echo -e "${BLUE}轻量级邮件服务器一键脚本 (智能API终极版 - 安全强化版)${NC}"
echo "=============================================================="
echo "请选择要执行的操作:"
echo "1) 安装邮件服务器核心服务"
echo "2) 卸载邮件服务器核心服务"
echo "3) 【推荐】配置域名反代和SSL证书 (Caddy)"
echo ""
