...
install_server() {
    ...
    read -p "请输入您的邮件域名 (例如: mail.yourdomain.com): " MAIL_DOMAIN
	if [ -z "$MAIL_DOMAIN" ]; then
		echo -e "${RED}错误：邮件域名不能为空。${NC}"
		exit 1
	fi
...
if __name__ == '__main__':
    ...
    ssl_ctx.load_cert_chain(
        certfile="/etc/letsencrypt/live/mail.dddd.com/fullchain.pem",
        keyfile="/etc/letsencrypt/live/mail.dddd.com/privkey.pem"
    )
    ...
    # --- 启动 SMTPS (465 端口, 直接加密) ---
    controller_ssl = Controller(
        CustomSMTPHandler(),
        hostname='0.0.0.0',
        server_hostname=MAIL_DOMAIN,
        port=465,
        ssl_context=ssl_ctx,   # 465 是隐式 TLS
        require_starttls=False
    )

    # --- 启动 Submission (587 端口, 可选 STARTTLS) ---
    controller_submission = Controller(
        CustomSMTPHandler(),
        hostname='0.0.0.0',
        server_hostname=MAIL_DOMAIN,
        port=587,
        ssl_context=ssl_ctx,   # 启用 STARTTLS
        require_starttls=False # 不强制
    )

    # 启动所有服务
    controller_plain.start()
    controller_ssl.start()
    controller_submission.start()

    app.logger.info("✅ SMTP 服务器已启动: 25(明文), 465(SMTPS), 587(Submission/STARTTLS 可选)")
...
Description=Custom Python SMTP Server (25, 465, 587)
After=network.target
...
ExecStart=/opt/mail_api/venv/bin/python3 /opt/mail_api/app.py
Restart=always
